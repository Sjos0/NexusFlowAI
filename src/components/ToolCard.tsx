      ...
      <div className="group p-3 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-1.5">
          <div className="flex items-center space-x-1 text-muted-foreground opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
            <IconButton onClick={onEdit} ariaLabel={`Editar ferramenta ${name}`} title={`Editar ferramenta ${name}`} className="hover:text-accent text-muted-foreground h-7 w-7">
              <Pencil size={16} />
            </IconButton>
            <IconButton onClick={onAddSubOption} ariaLabel={`Adicionar sub-opções para ${name}`} title={`Adicionar sub-opções para ${name}`} className="hover:text-accent text-muted-foreground h-7 w-7">
              <Plus size={16} />
            </IconButton>
            <IconButton onClick={onDelete} ariaLabel={`Deletar ferramenta ${name}`} title={`Deletar ferramenta ${name}`} className="hover:text-destructive text-muted-foreground h-7 w-7">
              <Trash2 size={16} />
            </IconButton>
          </div>
          <IconButton
            onClick={() => setIsOpen(!isOpen)}
            ariaLabel={isOpen ? `Recolher sub-opções de ${name}` : `Expandir sub-opções de ${name}`}
